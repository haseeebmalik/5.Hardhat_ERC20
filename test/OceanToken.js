...
describe("OceanToken contract", function() {
  ...
  describe("Deployment", function () {
    it("Should set the right owner", async function () {
      const s_owner=await oceanToken.getOwner()
      expect(owner.address).to.equal(s_owner);
    });
    ...
    it("Should set the blockReward to the argument provided during deployment", async function () {
      const blockReward = await oceanToken.s_blockReward();
  ...
  describe("Transactions", function () {
    ...
    it("Should update balances after transfers", async function () {
      ...
      const blockMinerAddress=await oceanToken.getBlockMiners();
      console.log("blockMinerAddress",blockMinerAddress)
      const blockMinerToken =await oceanToken.getBlockMinerTokens(blockMinerAddress[0])
      console.log("blockMinerToken",blockMinerToken.toString())
